...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'cek_stok':
        ...
        if not produk:
            await query.edit_message_text("Waduh, produk tidak ditemukan. Coba /start lagi.")
            return
...
@app.route('/')
def index():
    return "Skynexio Store Bot is alive!"
    ...
    if data.get('status') == 'PAID':
        ...
        if order:
            ...
            if akun:
                ...
                msg = "✅ Pembayaran diterima, tapi stok habis. Admin akan segera hubungi kamu."
