...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pesan = (
        ...
        f"Klik tombol di bawah untuk mulai belanja!"
    )
...
async def xendit_webhook():
    ...
    if data.get("status") == "PAID":
        ...
        if order:
            ...
            if akun:
                msg = f"✅ Pembayaran sukses!\n\nBerikut akun kamu:\n`{akun}`\n\nJika ada masalah, hubungi {ADMIN_USERNAME}"
                ...
                await bot_app.bot.send_message(order["user_id"], "Pembayaran berhasil, tapi stok habis. Admin akan hubungi kamu.")
    return jsonify({"status": "ok"})


# === SETUP DAN JALANKAN ===
async def setup():
    logger.info("🔧 Menjalankan setup()")
    ...
    logger.info("✅ Bot berhasil disiapkan dan webhook diatur")
