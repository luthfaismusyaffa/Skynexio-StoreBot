...
async def new_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update): return
    try:
        id_produk, nama, harga, deskripsi = " ".join(context.args).split("|")
        products = muat_data("products.json")
        if any(p['id'] == id_produk for p in products):
            await update.message.reply_text("❌ Produk sudah ada.")
            return
        products.append({
            "id": id_produk.strip(),
            "nama": nama.strip(),
        ...
        simpan_data(products, "products.json")
        await update.message.reply_text("✅ Produk berhasil dibuat.")
    except Exception as e:
        logger.error(e)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Hai! Untuk mulai, ketik /start")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'cek_stok':
        ...
        try:
            external_id = f"order-{produk_id}-{query.from_user.id}-{int(time.time())}"
            ...
            await context.bot.send_message(chat_id, f"✅ Invoice dibuat:\n{invoice.invoice_url}")
        except Exception as e:
            logger.error(e)
    ...
    try:
        ...
        if data.get("status") == "PAID":
            ...
            if order:
                ...
                if akun:
                    asyncio.run(bot_app.bot.send_message(order["user_id"], f"✅ Pembayaran diterima.\n\nBerikut akunmu:\n`{akun}`", parse_mode="Markdown"))
                    asyncio.run(bot_app.bot.send_message(ADMIN_CHAT_ID, f"✅ Penjualan: {akun}"))
                else:
                    asyncio.run(bot_app.bot.send_message(order["user_id"], "Pembayaran berhasil, tapi stok habis."))
        return jsonify({"status": "ok"})
    except Exception as e:
        logger.error(e)
        return jsonify({"status": "error"}), 500

# --- REGISTRASI HANDLER & RUN ---
async def setup():
