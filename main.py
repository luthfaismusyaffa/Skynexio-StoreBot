# main.py - VERSI FINAL DENGAN BALASAN OTOMATIS
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handler untuk semua teks yang bukan perintah."""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'cek_stok':
        ...
        if not keyboard: await context.bot.send_message(chat_id=chat_id, text="Yah, amunisi lagi kosong nih. 🙏"); return
        ...
        await query.message.delete()
        class MockMessage:
            def __init__(self, original_message): self.message = original_message
        ...
        try:
            ...
            orders = muat_data('orders.json'); orders.append({'external_id': external_id, 'user_id': update.effective_user.id, 'produk_id': produk_id, 'harga': produk['harga'], 'status': 'PENDING'})
...
# --- PERUBAHAN: Menambahkan handler untuk pesan teks biasa ---
